...
matrix_dir = "Data//"
barcode.path <- paste0(matrix_dir, "E-HCAD-31.aggregated_filtered_counts.mtx_cols")
features.path <- paste0(matrix_dir, "E-HCAD-31.aggregated_filtered_counts.mtx_rows")
matrix.path <- paste0(matrix_dir, "E-HCAD-31.aggregated_filtered_counts.mtx")
...
seurat_obj<- CreateSeuratObject(counts = mat, project = "E-HCAD-31", min.cells = 3, min.features = 100)

seurat_ids <- colnames(seurat_obj)

pheno_file=read.csv("ExpDesign-E-HCAD-31.tsv",sep = "\t")
...
saveRDS(srat,"31.seurat.org.rds",compress="xz")


data1=readRDS("31.seurat.org.rds")
data1$RNA
