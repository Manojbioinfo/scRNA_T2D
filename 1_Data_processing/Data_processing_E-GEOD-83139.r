...
matrix_dir = "Data//"
barcode.path <- paste0(matrix_dir, "E-GEOD-83139.aggregated_filtered_counts.mtx_cols")
features.path <- paste0(matrix_dir, "E-GEOD-83139.aggregated_filtered_counts.mtx_rows")
matrix.path <- paste0(matrix_dir, "E-GEOD-83139.aggregated_filtered_counts.mtx")
...
seurat_obj<- CreateSeuratObject(counts = mat, project = "E-GEOD-83139", min.cells = 3, min.features = 100)

seurat_ids <- colnames(seurat_obj)

pheno_file=read.csv("ExpDesign-E-GEOD-83139.tsv",sep = "\t")

pheno_file=dplyr::filter(pheno_file,Sample.Characteristic.disease.!="type I diabetes mellitus")
...
saveRDS(srat,"83139.seurat.org.rds",compress="xz")
